...
export default function ViolationDetails({ violation, onClose }: ViolationDetailsProps) {
  const [showConfirmModal, setShowConfirmModal] = useState<{ type: 'resolve' | 'warn'; violation: Violation } | null>(null);
  ...
  const confirmAction = () => {
    ...
    const messages = {
      resolve: `Violation ${violation.violationId} has been resolved successfully.`,
      warn: `Warning sent to ${violation.memberName} for violation ${violation.violationId}.`
    };
    ...
    <>
      <div className="absolute right-0 top-0 h-full w-96 bg-white border-l border-gray-200 shadow-xl overflow-y-auto">
        ...
        <div className="p-6 border-b border-gray-200 bg-gray-50">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1">
              <h4 className="font-medium text-gray-900">{violation.violationId}</h4>
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-gray-900">{violation.memberName}</span>
              </div>
              <div className="flex items-center space-x-2">
                <i className="ri-mail-line text-gray-400"></i>
                <span className="text-sm text-gray-600">{violation.memberEmail}</span>
              </div>
              <div className="flex items-center space-x-2">
                <i className="ri-home-4-line text-gray-400"></i>
                <span className="text-sm text-gray-600">{violation.memberUnit}</span>
              </div>
              <div className="flex items-center space-x-2">
          ...
          {/* Violation Details */}
          <div>
            <h5 className="font-medium text-gray-900 mb-3">Violation Description</h5>
            <p className="text-gray-700 text-sm leading-relaxed">{violation.fullDescription}</p>
          </div>

          {/* Report Information */}
          <div>
            <h5 className="font-medium text-gray-900 mb-3">Report Information</h5>
            <div className="bg-gray-50 rounded-lg p-4 space-y-2">
              <div className="flex items-center space-x-2">
                <i className="ri-calendar-line text-gray-400"></i>
                <span className="text-sm text-gray-600">
                  Reported on {new Date(violation.date).toLocaleDateString('en-US', { 
                    weekday: 'long', 
                    year: 'numeric', 
                    month: 'long', 
                    day: 'numeric' 
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-gray-600">Reported by {violation.reportedBy}</span>
              </div>
            </div>
          </div>
          ...
          {violation.resolutionNotes && (
            <div>
              <h5 className="font-medium text-gray-900 mb-3">Resolution Notes</h5>
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <p className="text-sm text-green-800">{violation.resolutionNotes}</p>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-900">Violation Reported</p>
                  <p className="text-xs text-gray-500">{new Date(violation.date).toLocaleDateString()}</p>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-4xl mx-4 max-h-[90vh] overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h3 className="text-xl font-semibold text-gray-900">Violation Report - {violation.violationId}</h3>
              <button
            ...
            <div className="p-8 overflow-y-auto max-h-[calc(90vh-120px)] bg-white">
              <div className="max-w-2xl mx-auto space-y-6">
                ...
                <div className="text-center border-b border-gray-200 pb-6">
                  <h1 className="text-2xl font-bold text-gray-900 mb-2">VIOLATION NOTICE</h1>
                  <p className="text-lg text-gray-600">{violation.violationId}</p>
                </div>

                {/* Violation Information */}
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-3">Resident Information</h3>
                    <div className="space-y-2 text-sm">
                      <p><span className="font-medium">Name:</span> {violation.memberName}</p>
                      <p><span className="font-medium">Email:</span> {violation.memberEmail}</p>
                      <p><span className="font-medium">Unit:</span> {violation.memberUnit}</p>
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      <p><span className="font-medium">Type:</span> {violation.violationType}</p>
                      <p><span className="font-medium">Status:</span> {violation.status.charAt(0).toUpperCase() + violation.status.slice(1)}</p>
                      <p><span className="font-medium">Date Reported:</span> {new Date(violation.date).toLocaleDateString('en-US', { 
                        weekday: 'long', 
                        year: 'numeric', 
                        month: 'long', 
                        day: 'numeric' 
                      })}</p>
                      <p><span className="font-medium">Reported By:</span> {violation.reportedBy}</p>
                    </div>
                  </div>
                </div>
                ...
                {/* Description */}
                <div>
                  <h3 className="font-semibold text-gray-900 mb-3">Violation Description</h3>
                ...
                {violation.resolutionNotes && (
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-3">Resolution Notes</h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-lg p-6 w-96 max-w-md mx-4">
            ...
            <p className="text-gray-600 mb-4">
              {showConfirmModal.type === 'resolve' && `Are you sure you want to resolve violation ${showConfirmModal.violation.violationId}?`}
              {showConfirmModal.type === 'warn' && `Send a warning notice to ${showConfirmModal.violation.memberName} for violation ${showConfirmModal.violation.violationId}?`}
            </p>
            ...
            <div className="flex space-x-3">
              ...
              <button
                ...
                className={`flex-1 py-2 px-4 rounded-lg font-medium text-white transition-colors whitespace-nowrap ${
                  showConfirmModal.type === 'resolve' ? 'bg-[#1FA372] hover:bg-[#188f5f]' : 'bg-amber-600 hover:bg-amber-700'
                }`}
              >
