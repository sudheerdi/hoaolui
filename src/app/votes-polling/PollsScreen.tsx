...
export default function PollsScreen() {
  ...
  const [selectedClosedPoll, setSelectedClosedPoll] = useState<string>('POLL-004');
  ...
  const mockPolls: Poll[] = [
    {
      id: 'POLL-001',
      title: 'Should we install new playground equipment in the community park?',
      type: 'open',
      status: 'active',
      totalVotes: 127,
  ...
  const selectedPollObjects = activeTab === 'active' 
  ...
  const currentClosedPolls = activeTab === 'closed' ? filteredPolls.slice(closedStartIndex, closedEndIndex) : [];
  ...
  const renderChart = (poll: Poll, index: number) => {
    ...
    return (
      <div key={poll.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-right">
            <div className="text-xl font-bold text-teal-600">{poll.totalVotes}</div>
          ...
          <svg viewBox="0 0 200 120" className="w-full h-full">
            ...
            {(() => {
              ...
              return poll.options.map((option, optionIndex) => {
                ...
                const segmentLength = (option.percentage / 100) * totalArcLength;
            ...
            {/* Center Text */}
            <text x="100" y="85" textAnchor="middle" className="text-lg font-bold fill-gray-900">
              {poll.totalVotes}
            </text>
            <text x="100" y="100" textAnchor="middle" className="text-xs fill-gray-600">
              Total Votes
            </text>
          </svg>
        </div>
        ...
        <div className="space-y-2">
          {poll.options.map((option, optionIndex) => (
            <div key={optionIndex} className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <span className="text-sm font-medium text-black">{option.percentage}%</span>
                <span className="text-sm text-black font-medium">{option.votes}</span>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-2">
                <span className="font-bold">Note: max 3 active polls allowed</span>
              </p>
            </div>
            <div className="flex items-center space-x-4">
              <div className="relative">
                ...
                {/* Create Poll Popover */}
                {showCreatePopover && (
                  <div className="absolute top-full right-0 mt-2 w-[600px] bg-white rounded-xl shadow-lg border border-gray-200 z-50">
                    {showSuccess ? (
                      <div className="p-8 text-center">
                        ...
                        <p className="text-gray-600">Your poll has been published and is now live for community voting.</p>
                      </div>
                    ) : (
                      <>
                        {/* Header */}
                        ...
                        <div className="p-4 border-b border-gray-200">
                          <div className="grid grid-cols-2 gap-3">
                            <div
                              onClick={() => setPollData(prev => ({ ...prev, type: 'anonymous' }))}
                            ...
                            >
                              <div className="flex items-start space-x-2">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-gray-600">Voters' identities are hidden</p>
                                </div>
                              </div>
                            </div>

                            <div
                              onClick={() => setPollData(prev => ({ ...prev, type: 'open' }))}
                              className={`p-3 border-2 rounded-lg cursor-pointer transition-colors ${
                                pollData.type === 'open'
                                  ? 'border-teal-500 bg-teal-50'
                                  : 'border-gray-200 hover:border-gray-300'
                            ...
                            >
                              <div className="flex items-start space-x-2">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-gray-600">Voters' identities are visible</p>
                                </div>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="p-6">
                          <div className="grid grid-cols-2 gap-6">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500 mb-3">Add at least 2 options for your poll</p>
              ...
              {selectedPollObjects.length > 0 && (
                <div className="px-4 mb-6">
                  <div className={`${
                    activeTab === 'closed' 
                      ? 'flex justify-center' 
                      : selectedPollObjects.length === 1 ? 'grid grid-cols-1 max-w-md' : 
                        selectedPollObjects.length === 2 ? 'grid grid-cols-2' : 
                        'grid grid-cols-3'
                  } gap-4`}>
                    {selectedPollObjects.map((poll, index) => renderChart(poll, index))}
                  </div>
                </div>
              )}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 mx-4 mb-6">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead style={{ backgroundColor: '#DCDCDC' }} className="border-b border-gray-200">
                      <tr>
                        <th className="px-4 py-3 text-left text-sm font-bold uppercase tracking-wider text-black">Topic</th>
                        <th className="px-4 py-3 text-left text-sm font-bold uppercase tracking-wider text-black">Type</th>
                        <th className="px-4 py-3 text-left text-sm font-bold uppercase tracking-wider text-black">Start Date</th>
                        <th className="px-4 py-3 text-left text-sm font-bold uppercase tracking-wider text-black">End Date</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                      {(activeTab === 'active' ? filteredPolls : currentClosedPolls).map((poll) => (
                        <tr
                          key={poll.id}
                          className={`hover:bg-gray-50 transition-colors ${
                            activeTab === 'closed' ? 'cursor-pointer' : ''
                          } ${
                            activeTab === 'closed' && selectedClosedPoll === poll.id 
                              ? 'bg-green-50 border-l-4 border-l-teal-600' 
                              : ''
                          }`}
                          onClick={() => activeTab === 'closed' && handleClosedPollSelection(poll.id)}
                        >
                          <td className="px-4 py-2">
                            <div className="text-base font-medium text-black">{poll.title}</div>
                            <div className="text-sm text-black font-medium">{poll.id}</div>
                          </td>
                          ...
                          <td className="px-4 py-2 whitespace-nowrap">
                            <span className="text-base text-black font-medium" suppressHydrationWarning={true}>
